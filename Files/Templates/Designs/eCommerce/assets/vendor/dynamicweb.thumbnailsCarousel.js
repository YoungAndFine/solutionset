  ...
  $.fn.dwThumbnailsCarousel = function (option) {
    return this.each(function () {
      ...
      var item_width
        ...
        , _carousel
        , _slider
        , $controlLeft = $(this).find('.carousel-control.left')
        , $controlRight = $(this).find('.carousel-control.right');
      ...
      // stop_pos = (visible_width*2) - total_width;
        ...
        'pln.prev': function() {
          if (_slider.position().left < 0 && _slider.position().left === _slider.data('to')) {
            ...
            _slider.animate({
              ...
              _slider.data('to', _slider.position().left);
              if (!$controlRight.is(':visible')) {
                $controlRight.toggle(true);
              }
            });
          } else {
            $controlLeft.toggle(false);
          }
          return false;
        },
        'pln.next': function() {
          if (_slider.position().left > stop_pos && _slider.position().left === _slider.data('to')) {
            ...
            _slider.animate({
              ...
              _slider.data('to', _slider.position().left);
              if (!$controlLeft.is(':visible')) {
                $controlLeft.toggle(true);
              }
            });
          } else {
            $controlRight.toggle(false);
          }
          return false;
        }
      });
